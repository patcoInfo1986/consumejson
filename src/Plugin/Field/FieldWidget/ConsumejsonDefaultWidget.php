...
namespace Drupal\consumejson\Plugin\Field\FieldWidget;
 ...
 *     label = @Translation("Consumejson default"),
 *     field_types = {
 *          "consumejson"
 *     }
 * )
 */
class ConsumejsonDefaultWidget extends WidgetBase {
    ...
    public function formElement(FieldItemListInterface $items, $delta, array $element, array &$form, FormStateInterface $form_state)
    {
     ...
     //   $uri = "http://etask.ebank/task/api/all";

        //FInd the Url s

      $uri = $this->getFieldSetting('jsonurl');
       // $uri = 'http://etask.ebank.org';
        $optionsArr[]  ="";
    
        $arrayUrl = get_headers($uri);
        $stringUrl = $arrayUrl[0];
        if(strpos($stringUrl,"200")){
                ...
                if(isset($data)){
                       ...
                       foreach ($row as $item){                                
                           $optionsArr[$item->field_nodeid] = $item->title;
                             }
